// Modified odom_subscriber to support multiple source positions.  This node
// reads a parameter `source.positions` (flattened list of [x,y,z] triples) and
// populates g_sources with an array of positions.  If the parameter is empty
// or malformed, the legacy single source (source.position) is used instead.
  ...
  stats_M2_(0.0)
{
  ...
  // Write selected particle to shared memory
  ...
  if (!src_positions.empty() && src_positions.size() % 3 == 0) {
    ...
    g_sources.clear();
    for (std::size_t i = 0; i < src_positions.size(); i += 3) {
      std::array<double, 3> pos{src_positions[i], src_positions[i + 1], src_positions[i + 2]};
      g_sources.push_back(pos);
    }
  }

  // Fallback to single source if no multi‑source parameter provided
  auto src_pos_single = this->declare_parameter<std::vector<double>>("source.position", {0.0, 0.0, 0.0});
  if (g_sources.empty() && src_pos_single.size() >= 3) {
    g_source_x.store(src_pos_single[0], std::memory_order_relaxed);
    g_source_y.store(src_pos_single[1], std::memory_order_relaxed);
    g_source_z.store(src_pos_single[2], std::memory_order_relaxed);
  }
  ...
  // Detector position and shield angle
  ...
  // Subscribe to /odom for robot kinematics
  ...
  // Publishers for energy deposition and statistics
  ...
  // Timer for periodic publishing of statistics
...
void OdomSubscriber::callback(const nav_msgs::msg::Odometry::SharedPtr msg)
{
  ...
  // Request one Geant4 event.  If multiple sources are defined, the
  // GeneratePrimaries() implementation will generate multiple vertices
  // accordingly.
...
{
  // Retrieve number of events processed since last check
  long b_cur = g_b.load(std::memory_order_relaxed);
  if (b_cur <= last_b_seen_) return;

  // Retrieve the most recent energy deposition
  ...
  // Publish instantaneous energy deposition
  std_msgs::msg::Float64 edep_msg;
  edep_msg.data = x; // [MeV]
  edep_pub_->publish(edep_msg);

  // Publish statistics: [count, mean, variance]
